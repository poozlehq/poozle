...
import { GetTeamParams, TeamResponse, UpdateTeamParams } from './team.interface';
import { convertTeam, teamMapping } from './team.utils';

export class TeamPath extends BasePath {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async getTeam(url: string, headers: AxiosHeaders, _params: GetTeamParams): Promise<TeamResponse> {
  ...
  async run(
    method: string,
    headers: AxiosHeaders,
    params: GetTeamParams | UpdateTeamParams,
    config: Config,
  ) {
    ...
    switch (method) {
      ...
      case 'PATCH':
        return this.updateTeam(url, headers, params as UpdateTeamParams);
